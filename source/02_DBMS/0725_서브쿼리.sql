-- [VI] SUB QERUY: 메인 QUERY(SQR문) 안에 QUERY(SQR문)가 내포
--★1.서브 쿼리 개념 
    --서브쿼리가 필요한 EX.급여를 제일 많이 받는 사람의 모든 정보
SELECT MAX(SAL)FROM EMP;
SELECT * FROM EMP WHERE SAL =(SELECT MAX(SAL) FROM EMP);

--서브쿼리의 종류(1) 단일행 서브쿼리 (서브퀄리의 실행 결과가 단일행) : =, > ,>=, <=, !=
--EX. SCOTT과 동일한 부서에서 근무하는 사원의 이름
 ...
 SELECT ENAME, SAL
  ...
  SELECT DEPTNO FROM EMP WHERE ENAME IN ('SCOTT', 'KING'); --다중행 서브쿼리10,20
  SELECT ENAME, SAL
    FROM EMP
    WHERE DEPTNO =(SELECT DEPTNO FROM EMP WHERE ENAME IN  ('SCOTT', 'KING'));--메인쿼리
    
--★2.단일행 서브쿼리
 --EX. SCOTT과 동일한 근무지에서 근무하는 사원의 모든 정보를 출력
  --데이터 추가 (DALLS 50번부터, 50번부터 근무하는 홍길동 사원)
...
INSERT INTO DEPT VALUES (50, 'IT', 'DALLAS');
 ...
 SELECT * FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND LOC=(SELECT LOC
        ...
        AND ENAME <> 'SCOTT'; --메인 쿼리
--EX2. 최초입사직원이름과 최소입사일
SELECT MIN(HIREDATE) FROM EMP; --서브쿼리(최소입사일)
 SELECT ENAME, HIREDATE FROM EMP EMP
WHERE HIREDATE =(SELECT MIN(HIREDATE)FROM EMP);-메이쿼리
--eX.  최근입사직원이름과 죄근 ㅇ비사일
SELECT MAX(HIREDATE) FROM EMP;
SELECT ENAME, HIREDATE FROM EP WHERE HIREDATE- (SELECT MAX(HIREDATE) FROM EMP);
--ex3. 최소이비사직원이름과 최최입사일, 최근입사작원 이름과 최근입사일(EX2 +EX3)
SELECT E1.ENAME , E1.HIREDATE ,E2.ENAME, E2.HIREDATE
    FROM EMP E1 ,EMP E2
    WHERE E1.HIREDATE =(SELECT MIN(HIREDATE) FROM EMP) AND
          E2.HIREDATE =(SELECT MAX(HIREDATE)FROM EMP);
SELECT 
    (SELECT ENAME FROM EMP WHERE HIREDATE= (SELELCT MIN(HIREDATE) FROM EMP)최초입사자,
    (SELECT MIN(HIREDATE) FROM EMP 최초입사일 ,
    (SELECT ENAME FROM EMP WHERE HIREDATR= (SELECT MAX(HOREDATE),FROM EMP)) 최근입사자
...
--EX7.DALLAS에 근무하는 사원의 이름과 부서번호(서브 쿼리,JOIN)
...
--ex11. 평균급여 이하로 받는 사원의 이름, 급여,평균급여와의 차이(소수점 1자리 반올림)을 출력
    SELECT ENAME , SAL, ROUND((SELECT AVG(SAL)FROM EMP)-SAL ,1) DIFF
        FROM EMP
     WHERE SAL <= (SELECT AVG(SAL) FROM EMP); 
--단일행 다중열 서브쿼리
--SCOTT의 JOB과 부서번호가 같으 ㄴ직원의 모든 필드 출력
...
--탄탄1. 부서별로 가장 급여를 많이 받는 사원의 모든 정보를 출력(IN연산자 이용)
...
-- 탄탄3. 급여가 3000이상인 사람들 중 급여 등급을 나누어서 해당 등급별 최고 급여를 받는 사람들의 사번, 이름, 직업, 입사일, 급여, 급여등급을 출력
...
--10. CLARK와 같은 부서번호이며, 사번이 7698인 직원의 급여보다 많은 급여를 받는 사원들의 사번, 이름, 
--급여
SELECT EMPNO, ENAME, SAL FROM EMP 
     WHERE DEPTNO= (SELECT DEPTNO FROM EMP WHERE ENAME='CLARK') 
     AND SAL>(SELECT SAL FROM EMP WHERE EMPNO=7698);
--11. CLARK와 같은 부서명이며, 사번이 7698인 직원의 급여보다 많은 급여를 받는 사원들의 사번, 이름, 급
--여
...
SELECT ENAME, JOB, SAL 
    FROM EMP  
    WHERE EMPNO IN(SELECT EMPNO FROM DEPT WHERE INITCAP(LOC)='DALLAS');
...
SELECT ENAME,JOB 
    FROM EMP
    WHERE DEPTNO = (SELECT DEPTNO FROM EMP WHERE DNAME = 'SALES'); 
...
SELECT ENAME, JOB, SAL FROM EMP WHERE SAL = ANY(SELECT SAL FROM EMP WHERE ENAME IN('SCOTT','WARD'));
