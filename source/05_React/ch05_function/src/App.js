              ...
              if(selectedId!==0 && window.confirm('삭제시 복구 불가. 삭제하시겠습니까?')){
                ...
                for(var idx=0; idx<_contents.length; idx++){
                  if(_contents[idx].id === selectedId){
                    _contents.splice(idx,1); //idx번째1개제거
                    setContents(_contents);//hook변경
                    if(_contents.length >0){ //_content.length가0 :welcome모드
                      setMode('read');  //hook변경
                      setSelectedId(_contents[0].id);//맨처음
                      // setSelectedId(_contents[_contents.length-1].id);//맨마지막으로
