...
public class Speaker implements IVolume {
	...
	public void volumeUp() {
		...
		volumeLevel++;
		System.out.println("SPEAKER 볼륨을 1 만큼 올려 현재 볼륨은 " + volumeLevel);
	}else {
		System.out.println("SPEAKER 볼륨이 최대치 (" + IVolume.SPEAKER_MAX_VOLUME+")여서 올리지 못함");
		}
	}
	@Override
	...
	public void volumeDown(int level) {
		if(volumeLevel-level> SPEAKER_MIN_VOLUME) {
			volumeLevel-=level;
			System.out.println("SPEAKER볼륨은"+level+ "만큼 내려 현재 볼륨은"+ volumeLevel);
		}else {
