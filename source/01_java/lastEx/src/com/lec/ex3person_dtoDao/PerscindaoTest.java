...
public class PerscindaoTest {
	public static void main(String[] args) {
		...
		System.out.println("2.직업별 출력");
		ArrayList<PersonDto> dtos = dao.selectJname("배우");
		if(dtos.isEmpty()) {
			System.out.println("배우 직원의 사람이 등록되지 않았습니다");
		}else {
		...
		System.out.println("2.개그맨직업별 출력");
			dtos = dao.selectJname("개그맨");
		if(dtos.size()==0) {
			System.out.println("개그맨 직원의 사람이 등록되지 않았습니다");
		}else {
		...
		System.out.println("3번. 전체 출력 테스트");
		dtos = dao.selectAll();
		if(dtos.isEmpty()) {
